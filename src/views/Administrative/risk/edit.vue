...
<script>
import { riskSave,riskUpdate,riskRead } from '@/api/risk'
...
export default {
    ...
    methods:{
        async init() {
           ...
           await riskRead(this.$route.params.id).then(res => {
               if(res !== 'error'){
                   console.log(res)
                   this.form = res.data;
                   this.form.fee_year = JSON.parse(this.form.fee_year)
                   this.handleCompanyChange(this.form.company_id)
        ...
        handleSubmit() {
            ...
            riskUpdate(this.$route.params.id,data).then(res => {
                if(res !== 'error'){
                    console.log(res)
