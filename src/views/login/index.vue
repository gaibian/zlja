...
export default {
  ...
  created(){
    ...
    let account = Lockr.get('reAccount');
    let password = Lockr.get('rePassword');
    if(account){
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) { //通过前端验证
          ...
          setTimeout(()=>{
            ...
            let data = {};
            data.user_name = this.loginForm.username;
            ...
            if(accountFlag){
              Lockr.set('reAccount',this.loginForm.username);
            }else{
              Lockr.rm('reAccount');
            }
            if(passwordFlag){
              Lockr.set('rePassword',this.loginForm.password);
            }else{
              Lockr.rm('rePassword');
            }
            login(data).then(res => {
              if(res !== 'error'){
                ...
                Lockr.set('user_name',res.data.userInfo.user_name)
            ...
            // axios.post('http://js.upingou.com/admin/base/login',data).then(res1 => {
            //   console.log(res)
            //   let res = res1.data
            //   if(res !== 'error'){
